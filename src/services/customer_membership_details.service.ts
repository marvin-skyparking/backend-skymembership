...
export async function updateAllMembershipRFIDByCustId(
  cust_id: number,
  rfid: string
): Promise<CustomerMembershipDetail[] | null> {
  try {
    // Find all membership records by cust_id
    const memberships = await CustomerMembershipDetail.findAll({
      where: { Cust_Member: cust_id }
    });

    if (memberships.length === 0) {
      throw new Error('No memberships found with the given cust_id');
    }

    // Loop through each membership and update the rfid field
    const updatedMemberships = await Promise.all(
      memberships.map(async (membership) => {
        membership.rfid = rfid;
        return await membership.save(); // Save the updated membership
      })
    );

    // Return the updated membership records
    return updatedMemberships;
  } catch (error) {
    console.error('Error updating membership RFIDs:', error);
    throw error;
  }
}

export async function getCustomerMembershipByRFID(
  rfid: string
): Promise<CustomerMembershipDetail | null> {
  try {
    // Find the customer membership record by RFID
    const membership = await CustomerMembershipDetail.findOne({
      where: { rfid: rfid }
    });

    // Return the membership record if found
    return membership;
  } catch (error) {
    console.error('Error retrieving membership by RFID:', error);
    throw new Error('Failed to retrieve membership by RFID');
  }
}
